...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        loss += np.sum(0.5 * self.reg * W1 ** 2) + np.sum(0.5 * self.reg * W2 ** 2)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for hidden_dim in hidden_dims:
            self.params['W' + str(cnt)] = weight_scale * np.random.randn(last_dim, hidden_dim)
        ...
        self.params['W' + str(cnt)] = weight_scale * np.random.randn(last_dim, num_classes)
